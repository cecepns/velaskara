...
import { Search, Plus, Calendar, Store, Clock, Award, FileText, ChevronLeft, ChevronRight, Edit, Trash2 } from 'lucide-react';
  ...
  return (
    <div className="max-w-6xl w-full mx-auto px-4 py-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-gray-900 font-display">Laporan Audit</h1>
          <p className="text-sm text-gray-500 mt-1">
            {user?.role === 'manager'
              ? 'Daftar riwayat audit operasional untuk outlet Anda'
              : 'Daftar audit, nilai kepatuhan, dan tanda tangan digital manager'}
          </p>
        </div>
      ...
      <div className="bg-white p-4 border border-gray-100 rounded-2xl shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Cari berdasarkan nama outlet atau auditor..."
            value={search}
            ...
            <table className="min-w-full divide-y divide-gray-100 text-left">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Tanggal & Shift</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Auditor</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Skor Kepatuhan</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Status TTD</th>
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider">Status Bayar</th>
                ...
                {audits.map((audit) => {
                  ...
                  return (
                    <tr key={audit.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {!isFailed ? (
                          <span className="text-xs text-gray-400 font-semibold">N/A (Gratis)</span>
                        ) : (
                          <span className={`inline-flex items-center text-xs font-semibold px-2.5 py-1 rounded-full ${isPaid ? 'bg-blue-50 text-blue-700' : 'bg-red-50 text-red-700'
                            }`}>
